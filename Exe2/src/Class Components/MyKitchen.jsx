...
export default class MyKitchen extends Component {
  ...
  CreateDishes = () => {
    data.map((item) =>
      this.ingredients.push(
        new Ingredient(item.id, item.name, item.image, item.calories)
      )
    );
    const pad_thai_ingr = [
      this.ingredients[0],
    ...
    const pad_thai = new DishRecipe(
      1,
      "Pad Thai",
      pad_thai_ingr,
      60,
      "stir-fry dish, mix all ingredients together",
      "https://glebekitchen.com/wp-content/uploads/2016/11/easypadthaibowltop-1-500x500.jpg"
    );
    const chop_suey = new DishRecipe(
      2,
      "Chop Suey",
      chop_suey_ingr,
      40,
      "5 minute stir fry,add each vegetable in the order in which they cook.",
      "https://kirbiecravings.com/wp-content/uploads/2020/04/chop-suey-5.jpg"
    );
    const pho = new DishRecipe(
      3,
      "Pho",
      pho_ingr,
      30,
  ...
  changeRecipes = (recipe) => {
    ...
    this.setState({
      recipes: newRecipes,
      counter_recipes: this.state.counter_recipes - 1,
    });
    this.setState({
      ready_to_eat: [...this.state.ready_to_eat, recipe],
      counter_ready: this.state.counter_ready + 1,
    });
  };
  //add recipe to made recipes list
  changeMadeRecipes = (recipe) => {
    const newRecipes = this.state.ready_to_eat.filter(
      (rec) => rec.id !== recipe.id
    );
    this.setState({
      ready_to_eat: newRecipes,
      counter_ready: this.state.counter_ready - 1,
    });
    this.setState({
    ...
    return (
      <div>
        ...
        <Recipes
          ...
          clickEvent={this.changeRecipes}
          counter={this.state.counter_recipes}
          btnColor={"btn-outline-primary"}
        />
        {/* Recipes ready to eat */}
        <Recipes
          recipes={this.state.ready_to_eat}
          title={"Ready to EAT!"}
          btnText={"eat!"}
          clickEvent={this.changeMadeRecipes}
          counter={this.state.counter_ready}
